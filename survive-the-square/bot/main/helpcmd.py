...
class Help(commands.DefaultHelpCommand):
    ...
    def get_command_signature(
        self, command: commands.Command, ignore_aliases: bool = False
    ) -> str:
    ...
    async def send_bot_help(
        self,
        cogs: typing.Dict[commands.Cog, typing.Iterable[commands.Command]],
        description: str = "",
        title: str = "Help",
    ):
        """Send help for the entire bot."""
        self.context.help_command_check = True
        e = discord.Embed(title=title, color=0x50C878, description=description)
        for cog in cogs:
            if not cog:
                continue
            lines = []
            for command in await self.filter_commands(cog.walk_commands()):
                if command.hidden:
                    continue
                signature = self.get_command_signature(command, ignore_aliases=True)
