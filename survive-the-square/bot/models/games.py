...
NO_PERMS = discord.PermissionOverwrite(read_messages=False, send_messages=False)
READ_PERMS = discord.PermissionOverwrite(read_messages=True, send_messages=False)
WRITE_PERMS = discord.PermissionOverwrite(read_messages=True, send_messages=True)
...
class Game(BaseModel):
    ...
    def player_list(self) -> str:
        ...
        for side in (1, 2):
            ...
            players = (
                Player.select()
                .join(GameMember)
                .where(GameMember.game == self, GameMember.side == side)
            )
            for player in players:
                ign = player.steam_name if self.is_steam else player.mobile_name
    ...
    async def new_role(self, guild: discord.Guild, name: str) -> discord.Role:
        ...
        return await guild.create_role(name=f"{self.name} - {name}", mentionable=True)
    ...
    async def setup(self, guild: discord.Guild):
        ...
        self.observer_channel_id = (
            await category.create_text_channel(
                ...
                overwrites={
                    **base_perms,
                    observer_role: WRITE_PERMS,
                    player_role: WRITE_PERMS,
                },
            )
        ).id
        self.player_channel_id = (
    ...
    def player_can_join(
        self, ctx: commands.Context, user: UserData, player: Player
    ) -> bool:
    ...
    async def add_player(self, ctx: commands.Context, user: discord.Member = None):
        ...
        channel_id = (
            await side_channel.category.create_text_channel(
        ...
        GameMember.create(player=player, game=self, side=side, channel_id=channel_id)
    ...
    async def remove_player(self, ctx: commands.Context, user: discord.Member = None):
        ...
        if member := self.get_member(player):
            side_role = ctx.guild.get_role(
                self.side_1_role_id if member.side == 1 else self.side_2_role_id
            )
            player_role = ctx.guild.get_role(self.player_role_id)
...
class GameMember(BaseModel):
    ...
    @classmethod
    async def convert(cls, ctx: commands.Context, raw_argument: str) -> GameMember:
        """Get a member of the game this command was run for."""
        game = Game.get_or_none(Game.category_id == ctx.channel.category_id)
        if not game:
            raise commands.BadArgument("This command must be run in a game channel.")
        ...
        if len(matches) == 1:
            return matches[0]
        if not matches:
            raise commands.BadArgument(f"No game members found by search {search!r}.")
        raise commands.BadArgument(f"Multiple game members found by search {search!r}.")
    ...
    async def name_matches(self, guild: discord.Guild, search: str) -> bool:
        ...
        return any(search in re.sub(r"[^a-z0-9]", "", name.lower()) for name in names)
