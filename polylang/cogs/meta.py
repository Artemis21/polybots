        ...
        for cog in cogs:
            for command in cogs[cog]:
                if command.hidden:
                    continue
                text += "**{}** *{}*\n".format(
                    self.get_command_signature(command), command.brief or Help.brief
                )
        e = discord.Embed(title="Help", color=0x00FF66, description=text)
        await self.get_destination().send(embed=e)

    async def send_command_help(self, command):
        desc = (command.help or Help.help).replace("{{pre}}", bot.command_prefix)
...
class Meta(commands.Cog):
    ...
    @commands.command(
        brief="Info on the bot.", description="Find out a little more about the bot."
    )
    async def about(self, ctx):
        embed = discord.Embed(color=colours["purple"], description=OTHER, title="About")
        embed.set_thumbnail(url=str(bot.user.avatar))
        embed.add_field(name="Author", value=AUTHOR)
        embed.add_field(
            name="Invite", value=f"**[Click Here]({INVITE.format(bot.user.id)})**"
        )
        embed.add_field(
            name="Source code", value=f"**[Click Here]({SOURCE.format(bot.user.id)})**"
        )
    ...
    async def stats(self, ctx):
        ...
        embed.set_thumbnail(url=str(bot.user.avatar))
        embed.add_field(
            name="Shard", value=f"#{bot.shard_id or 1} of {bot.shard_count or 1}"
        )
        embed.add_field(
