...
class Players(commands.Cog):
    ...
    async def player(
        self, ctx: commands.Context, *, user: typing.Optional[discord.Member] = None
    ):
        ...
        games = len(
            models.GameMember.select().where(models.GameMember.player == player)
        )
    ...
    @commands.command(
        brief="Add to your tribes.", name="add-tribes", aliases=["add-tribe", "at"]
    )
    ...
    @commands.command(brief="Set your mobile name.", name="mobile-name", aliases=["mn"])
    ...
    @commands.command(brief="Set your steam name.", name="steam-name", aliases=["sn"])
    ...
    async def set_timezone(self, ctx: commands.Context, timezone: timezones.Timezone):
    ...
    @commands.command(brief="Search a player by ign.", aliases=["lookup", "s"])
    async def search(self, ctx: commands.Context, *, in_game_name: str):
        ...
        if not matches:
            await ctx.send(f"No player found by in-game name `{in_game_name}`.")
            return
        lines = []
        for match in matches:
            ...
            lines.append(f"<@{match.discord_id}> - {names}")
        await ctx.send(
            embed=discord.Embed(
                title="Possible matches", description="\n".join(lines), colour=0xF58F29
            )
        )
