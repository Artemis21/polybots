...
class Games(commands.Cog):
    ...
    @commands.command(brief="Create a game.", name="new-game", aliases=["ng", "new"])
    @checks.admin
    async def new_game(self, ctx: commands.Context, platform: str, limit: int = 14):
        ...
        game.save()
        await ctx.send(f"Created game {game.id} (<@&{config.WAITLIST_ROLE_ID}>).")

    @commands.command(brief="Join a game.", name="join-game", aliases=["j", "join"])
    ...
    @commands.command(brief="Leave a game.", name="leave-game", aliases=["l", "leave"])
    async def leave_game(self, ctx: commands.Context, game: models.Game):
        ...
        if game.is_open:
            ...
            await ctx.send(f"{game.name} is closed. Contact an admin to remove you.")
    ...
    async def add_member(
        self, ctx: commands.Context, user: discord.Member, game: models.Game
    ):
    ...
    @commands.command(
        brief="Remove a user from a game.",
        name="remove-member",
        aliases=["r", "remove"],
    )
    @checks.admin
    ...
    @commands.command(brief="Close a game.", name="close-game", aliases=["close", "c"])
    @checks.admin
    ...
    async def game(self, ctx: commands.Context, game: models.Game = None):
        ...
        players = (
            ...
            .join(models.GameMember)
            .where(models.GameMember.game == game)
        )
        player_lines = []
        for player in players:
            ign = player.steam_name if game.is_steam else player.mobile_name
            player_lines.append(f"<@{player.discord_id}> - `{ign}`")
