...
class Timezone:
    ...
    @classmethod
    def from_hours(cls, hours: int) -> Timezone:
        """Get an offset from a signed int representing hours."""
        return cls(hours < 0, abs(hours), 0)
...
class TimezoneField(peewee.Field):
    ...
    Stores offsets as a single as follows (most to least significant bits):
    0      - Always a low bit.
    ...
    def db_value(self, timezone: Timezone) -> Optional[int]:
    ...
    def python_value(self, value: int) -> Optional[Timezone]:
